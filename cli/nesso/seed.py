...
def check_seed_exists(seed_name: str) -> bool:
    """Runs `dbt ls` to check if `seed_name` is already visible to dbt as a seed.
    This means that the seed is added to the schema YAML.

    Note that this does not mean that the seed is materialized.

    Args:
        seed_name (str): The name of the seed to check.

    Returns:
        bool: Whether the seed is "registered" in dbt (added to schema YAML).
    """
...
@run_in_dbt_project
def register(
    seed_path: str = typer.Argument(..., help="The path to the seed file to be registered."),
    ...
    schema_path: str = typer.Option(
        ...
        "--yaml-path",
        help="The absolute path of the schema file to which to append seed schema, default path is DBT_PROJECT_DIR/seeds/master_data/schema.yml",
    ),
    ...
    Args:
        seed (str, Optional): The name of the seed to register, if not specified all seeds will be
            registered.
    ...
    # If the seed isn't registerd in dbt (is not present in the schema YAML), we
    # first materialize it in the database, and then generate the YAML and add it to the schema YAML.
