...
# In github action, tests are run outside the "data platform test" container, so the postgres database must be located on localhost

if "data_platform_test" not in containers_names:
    HOST = "localhost"
